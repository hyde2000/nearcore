...
fn test_wasmer2_artifact_output_stability() {
    ...
    let expected_hashes = [
        18202109167614012273,
        6447014787838102836,
        11697107130663444012,
        12595491324983981419,
        16474677138972935760,
        9454591335905396305,
        1559551864276181101,
    ];
    let seeds = [2, 3, 5, 7, 11, 13, 17];
    let input_hashes = [
        13685187424711381356,
        14874064587977634803,
        14533057556711812527,
        878586650292194527,
        3964709159845744664,
        18149828446803675487,
        4708788108836517955,
    ];
    let mut got_input_hashes = Vec::with_capacity(seeds.len());
    let mut got_hashes = Vec::with_capacity(seeds.len());
    for seed in seeds {
        let mut hasher = StableHasher::new();
        let contract = near_test_contracts::arbitrary_contract(seed);
        let mut ch = StableHasher::new();
        contract.hash(&mut ch);
        let config = VMConfig::test();
        let prepared_code = prepare::prepare_contract(&contract, &config).unwrap();
        prepared_code.hash(&mut ch);
        got_input_hashes.push(ch.finish());
        ...
        std::fs::write(format!("/tmp/artifact{}", got_hashes[0]), serialized).unwrap();
    }
    ...
    assert_eq!(got_input_hashes, input_hashes, "contract preparation has changed");
    ...
    assert_eq!(got_hashes, expected_hashes, "VM output has changed");
